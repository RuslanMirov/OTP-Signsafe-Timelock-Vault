...
describe("Holder", function () {
  ...
  describe("setNewPassword", function () {
    it("non-owner cannot set password", async function () {
      await expect(holder.connect(userTwo).setNewPassword(passHash(NEW_PASS)))
        .to.be.revertedWith("Ownable: caller is not the owner");
    });
    it("owner can rotate password and OTP resets", async function () {
      // первый раз используем OTP
      const { signature } = await signTransfer(owner, domain, userTwo.address, PASS);
      await holder.setNewOwner(userTwo.address, PASS, signature);
      expect(await holder.isPassUsed()).to.equal(true);

      // новый owner меняет пароль
      await holder.connect(userTwo).setNewPassword(passHash(NEW_PASS));
      expect(await holder.isPassUsed()).to.equal(false);
    });
    it("old signature fails after password rotation", async function () {
      const { signature } = await signTransfer(owner, domain, userTwo.address, PASS);
      await holder.setNewPassword(passHash(NEW_PASS));
      await expect(holder.setNewOwner(userTwo.address, PASS, signature))
        .to.be.revertedWith("OTP: wrong password");
    });
